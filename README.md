...
Every production deployment of Photomap — Dreamhost or otherwise — starts the same way: run one
script locally to build the frontend and stage a production-ready backend, then upload the
result. This step itself is entirely generic Apache/PHP/MySQL shared-hosting shaped, but works
just as well as the source for any other plain PHP+MySQL host.
...
- `release/domain.com/` — the frontend's `dist/` build plus a Dreamhost-flavored `.htaccess` and
  `api/index.php` stub (works for any single-domain-docroot Apache host, not just Dreamhost).
...
A worked, concrete example of the generic flow above on Dreamhost-style shared hosting — one
Apache-mapped directory per domain, no reverse proxy you control, SFTP/SSH as the only upload
mechanism. Full details, rationale, and the file-by-file layout live in `backend/README.md`'s
"Deploying to Dreamhost" section. Host-specific steps on top of "Building a release" above:

- In the Dreamhost panel, confirm/select PHP ≥ 8.1 for the domain, with the extensions
  `pdo_mysql`, `gd`, `fileinfo`, `curl`, `json`, `mbstring`, `exif` available, before uploading.
- The DB host/name/user/password you fill into `config.php` come from Dreamhost's panel (MySQL
