...
## Building a release (host-agnostic)

Every production deployment of Photomap starts the same way: run one script locally to build the
frontend and stage a production-ready backend, then upload the result. This step itself is
entirely generic Apache/PHP/MySQL shared-hosting shaped, but works just as well as the source for
any other plain PHP+MySQL host.
...
Prerequisite: Node/npm, plus either Composer or Docker locally (if Composer isn't on `PATH`, the
script automatically falls back to running it via the official `composer:2` Docker image). This
produces, at the repo root:

- `release/photomap-backend/` — the backend project (curated: excludes tests/Docker/`.env*`),
  including a freshly built production `vendor/`.
- `release/domain.com/` — the frontend's `dist/` build plus a generic shared-hosting-flavored
  `.htaccess` and `api/index.php` stub (works for any single-domain-docroot Apache host).

`release/` is gitignored — a build artifact regenerated by this command, never committed.

### Upload and run migrations

Once you have a `release/` tree:

1. Upload `release/photomap-backend/` to a **private** directory outside your domain's mapped
   docroot (e.g. via SFTP to `~/photomap-backend/`), and the **contents** of `release/domain.com/`
   to your domain's docroot (e.g. `~/yourdomain.com/`).
2. Copy `backend/config.php.example` to `config.php` inside the uploaded `photomap-backend/`
   directory (either locally before packaging, or directly on the host) and fill in your host's
   real DB host/name/user/password, a generated `APP_SECRET`, and an absolute `STORAGE_PATH` (a
   sibling of `photomap-backend/`, e.g. `~/photomap-backend/storage`) — see the file's inline
   comments.
3. **Only if there are new or pending migrations** (always true on a first deploy; true on a later
   update only when it added new files under `backend/migrations/`), run `php scripts/migrate.php`
   once, over SSH, inside the uploaded `photomap-backend/` directory. It's idempotent (tracks
   applied files in `schema_migrations`), so running it when nothing is pending is a safe no-op —
   but it's not something you need to run on every deploy, only when migrations changed. No SSH?
   A phpMyAdmin-import fallback (concatenating `migrations/*.sql` into one file) is documented in
   `backend/README.md`, for first-time setup only.
4. Load your domain in a browser and confirm `/`, `/share/:token`, and an upload/share round-trip
   all work.

## Deploying to a shared host (Apache + PHP + MySQL)

A worked, concrete example of the generic flow above on typical Apache + PHP + MySQL shared
hosting — one Apache-mapped directory per domain, no reverse proxy you control, SFTP/SSH as the
only upload mechanism. Full details, rationale, and the file-by-file layout live in
`backend/README.md`'s "Deploying to a shared host" section. Host-specific steps on top of
"Building a release" above:

- In your host's control panel, confirm/select PHP ≥ 8.1 for the domain, with the extensions
  `pdo_mysql`, `gd`, `fileinfo`, `curl`, `json`, `mbstring`, `exif` available, before uploading.
- The DB host/name/user/password you fill into `config.php` come from your host's panel (MySQL
  hosting section); `backend/README.md` shows the exact fields.
...
- **HTTPS ordering**: if login appears to succeed but you're immediately signed back out (the
  ...
  enabling HTTPS (Let's Encrypt or another TLS cert) for the domain. `Secure`-flagged cookies are
  silently dropped by the browser over plain HTTP, with no visible error. Enable HTTPS first, or
  use `APP_ENV=development` temporarily while testing over plain HTTP.
- **`mod_userdir` exposure**: the private backend directory's non-web-reachability relies on it
  being outside any domain's mapped docroot — but some shared hosts (depending on account
  settings) also serve account home directories at
...
`npm run deploy` (equivalent to `bash backend/scripts/deploy.sh`) builds the release (reusing
`npm run release`/`package-for-deploy.sh`) and then either uploads it automatically or prints a
concrete manual checklist, depending on whether a `deploy.config.json` is present at the repo
root:

1. Copy `deploy.config.example.json` to `deploy.config.json` (repo root — gitignored, never
   committed) and fill in `sshHost`, `sshPort`, `sshUser`, `sshKeyPath`, `remoteBackendPath`,
   `remoteFrontendPath`, `remoteMigrateCommand`, and optionally `postDeployCommands`.
2. **Key-auth only, by design**: the automated path only ever runs when `sshKeyPath` points to a
   readable private key file. Password-based automation is deliberately never attempted, for
   security reasons — if you only have password auth to your host, use the numbered manual
   fallback instructions `deploy.sh` prints instead (the same steps as "Upload and run
   migrations" above).
3. When a fully-populated, key-auth config is found, `deploy.sh` `chmod 600`s it automatically,
   then uses `rsync` over SSH (falling back to `scp -r` if `rsync` isn't available) to upload the
   backend and frontend directories, and finally runs `remoteMigrateCommand` (plus any
   `postDeployCommands`) over SSH, printing real output from each step.
4. If the config is missing or incomplete, `deploy.sh` prints the same numbered manual
   SFTP/SSH checklist described above, using whatever concrete values the config does specify.

The fallback (no-config) path, the missing-required-field path, and automatic `chmod 600`
enforcement are all covered by manual testing in this repo's dev environment (which has no local
SSH server available). The `rsync`/`scp`-over-`ssh` automation path itself was code-reviewed but
not live-tested against a real SSH target for that reason — verify it against your actual host
the first time you use it, before relying on it for a routine deploy.
...
deploy/shared-hosting/  Shared-hosting deploy assets: domain-docroot .htaccess, the api/index.php
                     stub, and an on-demand Apache-routing verification script (see "Deploying
                     to a shared host" above and backend/README.md)
deploy.config.example.json  Template for `npm run deploy`'s optional automated upload config
backend/scripts/deploy.sh   Builds a release, then automates upload+migrate via rsync/ssh if a
                     deploy.config.json (gitignored) is present, else prints manual instructions
